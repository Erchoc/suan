...
import { kpMap, type KPWithContext } from '../data/kpIndex';
import type { KnowledgeCard, Question, PreviewSession } from '../types';
import { useQuestions } from './useQuestions';

export interface PreviewSessionData {
  session: PreviewSession | null;
  currentKp: KPWithContext | null;
  currentCard: KnowledgeCard | null;   // null → 触发 fallback UI
  currentQuestions: Question[];        // 按 questionCount 截取
...
export function usePreviewSession(sessionId: string): PreviewSessionData {
  ...
  return useMemo(() => {
    ...
    const questionCount = isJunior ? Math.min(3, allKpQuestions.length) : Math.min(8, allKpQuestions.length);
