...
这些键属于兼容契约，项目更名或重构时不能随意改动。当前已经有家长身份与服务端登录会话，但学习记录仍没有云端持久化；不能把“已登录”描述成“已跨设备同步”。

## 六、当前接口

- `GET /api/health`：报告 Worker 与 AI 配置状态。
- `/api/auth/*`：手机号验证码、微信网站扫码、用户会话读取/撤销；服务商未配置时入口自动隐藏。
...
题库管理已经使用 D1 实现，并保留历史题面。手工修改、启停和 AI 质检结果自动同步给后续新建作答；AI 生成题默认停用，质检合格后才启用。家长账号已建立 `users` 与手机号/微信身份体系，验证码和会话都只存摘要。考试与复习 Session 仍保存在浏览器本地，但会直接固化完整题目和题库版本，所以后续修题不会改写既有试卷或做题记录。孩子档案、学习 Session 云端化、OCR 与周报接口仍未实现。
