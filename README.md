...
└── 同源 /api/* 请求
    └── Hono Cloudflare Worker
        ├── /api/health
        ├── /api/auth/* → 手机号短信 / 微信扫码 / HttpOnly 家长会话
        ├── /api/questions 与 /api/admin/*
        │   └── Cloudflare D1（题库、历史版本、用户反馈、AI 任务、用户身份与会话）
...
| `TENCENT_SMS_SECRET_ID` / `TENCENT_SMS_SECRET_KEY` | 空 | 可选；腾讯云短信 API 凭据，仅由 Worker 读取 |
| `TENCENT_SMS_SDK_APP_ID` | 空 | 可选；腾讯云短信应用 ID |
| `TENCENT_SMS_SIGN_NAME` / `TENCENT_SMS_TEMPLATE_ID` | 空 | 可选；审核通过的短信签名与单参数验证码模板 |
| `TENCENT_SMS_REGION` | `ap-guangzhou` | 可选；腾讯云短信请求地域 |
